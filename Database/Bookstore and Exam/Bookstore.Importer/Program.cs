...
namespace Bookstore.Importer
{
    public class Program
    {
        ...
        public static void Search()
        {
            ...
            {
                var queryInReview = db.Reviews.AsQueryable();
                ...
                if (xmlQuery.Attribute("type").Value == "by-period")
                {
                    ...
                    queryInReview = queryInReview.Where(r => r.CreatedOn >= startDate && r.CreatedOn <= endDate);
                }

                else if (xmlQuery.Attribute("type").Value == "by-author")
                {
                    var authorName = xmlQuery.Element("author-name").Value;
                    queryInReview = queryInReview.Where(r => r.Author.Name == authorName);
                }

                var resultSet = queryInReview
